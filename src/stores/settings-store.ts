...
/**
 * User preferences (Settings screen, Ctrl+,). Persisted per-setter through the
 * shared localStorage helpers like the data repositories — no middleware.
 * Applied to the document by `useApplySettings` (mounted in AppShell).
 */
...
interface SettingsState {
  ...
  readonly showVoiceBar: boolean;
}

interface SettingsStore extends SettingsState {
  setTextZoom(zoom: TextZoom): void;
  setReducedMotion(on: boolean): void;
  setShowVoiceBar(on: boolean): void;
...
export const useSettingsStore = create<SettingsStore>()((set, get) => {
  const persist = (patch: Partial<SettingsState>) => {
    set(patch);
    const { textZoom, reducedMotion, showVoiceBar } = get();
    saveJSON('settings', { textZoom, reducedMotion, showVoiceBar });
  };
  ...
  return {
    ...
    setShowVoiceBar: (showVoiceBar) => persist({ showVoiceBar }),
    reset: () => set(DEFAULTS),
  };
});

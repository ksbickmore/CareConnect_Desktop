...
export interface UseSpeechRecognition {
  ...
  error: string | null;
  /** False when the browser lacks a SpeechRecognition implementation. */
  available: boolean;
  start(): Promise<void>;
  stop(): void;
}
...
/**
 * React hook over the speech wrapper used by `VoiceInputBar`, `RecordingRadar`
 * and the dictation fields. `onFinal` fires once per completed utterance with
 * the final transcript. Ported from the mobile app's identical hook.
 */
